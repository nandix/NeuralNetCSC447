...
NeuralNet::NeuralNet( int nLayers, vector<int> nPerLayer )
{
	...
	for( int layer = 1; layer < numLayers; layer++ )
	{
		...
		wPrevious[ layer-1 ].resize( nPerLayer[layer] + 1 );
		...
		for( node = 0; node < nodesPerLayer[layer]; node++ )
		{
			...
			wPrevious[ layer-1 ][ node ].resize( nodesPerLayer[layer-1] + 1 );
			...
			for( int wNum = 0; wNum <= nodesPerLayer[layer-1]; wNum++ )
			{
				...
				wPrevious[ layer-1 ][ node ][ wNum ] = 0.0;
...
{
	// cout << " ------------- New Training Sequence ----------------" << endl;
	...
	vector< vector< vector<float> > > tempDeltas(numLayers-1); // Amount we updated this time
	...
	for( int layer=0; layer < numLayers-1; layer++ )
	{
		...
		tempDeltas[layer].resize( nodesPerLayer[layer+1] );
		...
		for( int to=0; to < nodesPerLayer[layer+1]; to++ )
		{
			...
			tempDeltas[layer][to].resize(numFrom, 0.0);
		}
	}
	...
	for( int layer = 0; layer < networkWeights.size(); layer++ )
		for( int to = 0; to < networkWeights[layer].size(); to++ )
			for( int from = 0; from < networkWeights[layer][to].size(); from++ )
			{
				tempDeltas[layer][to][from] =  updateDeltas[layer][to][from] + momentum * wPrevious[layer][to][from];
				networkWeights[layer][to][from] += tempDeltas[layer][to][from];
				// cout << "(" << layer << ", " << to << ", " << from << "): ";
				// cout << "Updating " << networkWeights[layer][to][from] 
				// 	<< " with momentum " << wPrevious[layer][to][from] 
				// 	<< " and update " << tempDeltas[layer][to][from]
				// 	<< endl;

			}

	wPrevious = tempDeltas;
}
